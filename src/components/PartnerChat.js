...
import { Paperclip, Mic, ImageIcon, SendHorizonal, FilePen } from "lucide-react";
...
export default function PartnerChat() {
  const [messages, setMessages] = useState([
    { id: 1, sender: "omnis", text: "Hello, Creator. How can I assist you today?", status: "sent", read: true },
  ]);
...
const toggleEmojiPicker = () => {
  ...
  // This effect will run whenever messages or typingStatus change
  ...
  const handleSend = () => {
    ...
    // simulate server confirm & bot reply
    setTimeout(() => {
      setMessages((prev) =>
  ...
  const handleFileUpload = (file) => {
    ...
    uploadTask.on(
      "state_changed",
      snapshot => {
        setFileUploadProgress((snapshot.bytesTransferred / snapshot.totalBytes) * 100);
  ...
  const handleAction = (action, msg) => {
    if (action === "trash")
      { // Compare with action type string
  ...
  return (
    <div className="flex flex-col dark:bg-[#0b0f19] h-[calc(100vh-4rem)] px-4 py-4 bg-gray-50">
      <div className="mb-2">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Converse with Omnis</p>
      </div>
  ...
  <div className="flex items-center relative">
    <textarea
      ...
      className="w-full resize-none rounded-xl border-gray-300 py-2 pl-10 pr-14shadow-sm 
